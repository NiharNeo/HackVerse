...
export function planRoute(startArea, endArea, era, sensory, trafficEngine, trafficFilters) {
  ...
  // 4. Compile stats for AuraPath (Serene walking lane avoids severe noise and emissions)
...
function findAuraPath(start, end, graph, era, sensory, trafficEngine, trafficFilters) {
  ...
  // A simplified Dijkstra's algorithm using dynamic edge weights
  const distances = {};
  const previous = {};
  ...
  while (unvisited.size > 0) {
    ...
    for (const neighbor of neighbors) {
      ...
      // Calculate the weight of traveling to this neighbor
...
function calculateLinkWeight(fromNode, toNode, era, sensory, trafficEngine, trafficFilters) {
  ...
  // Determine area name from label mapping
  ...
  // Get live predicted traffic metrics
  ...
  // 2. High Congestion Penalty: We strongly penalize gridlocked paths
  if (pred.congestionLevel > 80) cost += 150;
  else if (pred.congestionLevel > 50) cost += 60;
  else cost += pred.congestionLevel * 0.2;

  // 3. Sensory Mood Bonus (Subtract cost for good sensory fit)
  // Low noise (Acoustics slider represents quiet environments when value is low)
  const isAcousticCozy = sensory.acoustics < 0.4 && pred.congestionLevel < 40;
  if (isAcousticCozy) cost -= 4; // bonus

  // 4. Era suitability bonus (If there is a local destination matching this node and active era)
  ...
  if (hasEraMatch) cost -= 5; // bonus for cultural stopping points
...
function calculateStats(path, isAura, trafficEngine, trafficFilters) {
  ...
  // Time calculations: Aura path is a slow walking detour path (average speed is slower but peaceful)
  // Standard driving path suffers from traffic bottlenecks
  let duration = 0;
  if (isAura) {
    // Walking average (10 mins per node connection + sensory stopovers)
    duration = path.length * 8 + Math.round(avgCong * 0.1);
  } else {
    // Driving average: standard distance formula factored by severe congestion
  ...
  // Carbon Emissions: standard car emissions vs quiet walking
  const baseEmissions = parseFloat(((totalImpact / 100) * 0.45).toFixed(1));
  const carbon = isAura 
    ? parseFloat((baseEmissions * 0.05).toFixed(1)) // Walking has minimal carbon
    : parseFloat((baseEmissions * (1 + (avgCong / 100))).toFixed(1));

  // Acoustics / Decibels: heavy traffic noise vs quiet neighborhood lanes
  const decibels = isAura
    ? Math.round(40 + (avgCong * 0.15)) // Serene lanes stay quiet
    : Math.round(75 + (avgCong * 0.18)); // Main roads get extremely noisy
...
function getDestinationsForRoute(path, era, sensory) {
  ...
  return destinations.filter(dest => {
    // A. Must be located at an intersection along the path
    const destNode = dest.road.toLowerCase().replace(/\s+/g, '_');
    const isClose = pathNodes.has(destNode);
    if (!isClose) return false;

    // B. Suitability check for active historical era
    if (era !== 'all' && dest.era !== era) return false;

    // C. Sensory Suitability check: matches acoustic / lighting profile
    // (If user wants quiet, filter out extremely high acoustic cafes)
